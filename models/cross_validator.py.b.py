...
class CrossValidator:
    ...
    def save_results(self, prefix='cv_results', output_dir='G:/共有ドライブ/GAP_長寿研/user/iwamoto/視線の動きの俊敏さ/result'):
        """
        結果をファイルに保存
        ...
        prefix : str, default='cv_results'
            出力ファイル名の接頭辞
        output_dir : str, default='G:/共有ドライブ/GAP_長寿研/user/iwamoto/視線の動きの俊敏さ/result'
            結果ファイルの出力先ディレクトリ
        """
        if not self.results:
            print("まだ交差検証が実行されていません。")
...
def run_cv_analysis(df, model_class, n_splits=5, output_dir='G:/共有ドライブ/GAP_長寿研/user/iwamoto/視線の動きの俊敏さ/result'):
    """
    交差検証で分析を実行する関数
    ...
    # データの前処理
    print("データの前処理を開始...")
    features = df.drop(['ID', 'ncggfat_label'], axis=1)
